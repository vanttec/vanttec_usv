  ...
  switch(this->state){
    ...
    case 1:
      if(params.goal_reached){
        ...
        this->last_r.x = 0.0;
        this->last_r.y = 0.0;
        this->goal = forward(this->last_goal, 2.5);
        this->state = 2;
      }
      break;
    case 2:
      ...
      switch(choice){
        case 1:
          if((this->last_r.x != 0) && (this->last_r.y != 0)){
            mid_goal = USVPose{(int_buoy.x+2*this->last_r.x)/3,(int_buoy.y*2+this->last_r.y)/3,0};
...
bool FTP::is_new_buoy(std::vector<USVPose> reg, const USVPose &buoy){
  for(int i = 0 ; i < reg.size() ; i++){
    if(std::sqrt(std::pow(reg[i].x - buoy.x,2) + std::pow(reg[i].y - buoy.y,2)) < 2.5)
      return false;
  }
  return true;
}
