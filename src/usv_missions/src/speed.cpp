    ...
    case 1:
      if(params.goal_reached){
        ...
        if(this->yellow_found){
          this->goals.push_back(goal_trans(this->yellow_buoy, -2,0));
          this->goals.push_back(goal_trans(this->yellow_buoy, 0,1.5));
          this->goals.push_back(goal_trans(this->yellow_buoy, 1.5,0));
          this->goals.push_back(goal_trans(this->yellow_buoy, 0,-1.5));
          this->goal = goal_trans(this->starting_gate, -1.49,0);
          // this->goal = this->starting_gate;
          std::cout << "ROUNDING YELLOW BUOY!" << std::endl;
          this->state = 2;
        } else{
          this->goal = goal_trans(this->last_goal, 1.5, 0);
...
USVPose SPEED::goal_trans(const USVPose &goal, float x_t, float y_t){
  USVPose p{0,0,0};
  p.theta = goal.theta;
  if(x_t != 0){
    p.x = goal.x + x_t*std::cos(p.theta);
    p.y = goal.y + y_t*std::sin(p.theta);
  } else {
    p.x = goal.x - x_t*std::sin(p.theta);
    p.y = goal.y + y_t*std::cos(p.theta);
  }
  std::cout << "GOAL TRANSLATION TRAVEL: " << p.x << ", " << p.y << ", " << p.theta << std::endl;
  return p;
}
