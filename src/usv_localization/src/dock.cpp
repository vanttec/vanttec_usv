...
USVOutput DOCK::update(const USVPose &pose, const  USVUpdate &params){
  this->goals.clear();
  this->pose = pose;
  switch(this->state){
    case 0:
      // if(params.obs_list.size() > 1){
        ...
        if(!(this->goal.x == 0 && this->goal.y == 0 && this->goal.theta == 0)){
          //this->goals.push_back(forward(this->goal, -5.0));
          this->goals.push_back(this->goal);
          this->state = 1;
        }
        std::cout << "State 0 goal: " << this->goal.x << ", " << this->goal.x << std::endl;
        // std::cout << "Provided obj_list: " << params.obs_list[0].y << ", " << params.obs_list[1].y << ", " << params.obs_list[2].y << std::endl;
        std::cout << "Provided obj_list: " << params.obs_list.size() << std::endl;
      // }
      break;
    case 1:
...
USVPose DOCK::get_goal(std::vector<Obstacle> obs_list, int added_distance, const USVPose &pose){
  Gate g{0,0,0,0};
  USVPose p{0,0,0};
  USVPose buoy_t;
  ...
  std::cout << "OBJECTS: " << po(0) << "," << po(1) << std::endl;
  ...
  // CHOICES: 0 = circle, 1 = duck, 2 = plus, 3 = triangle
  // CHOICES: 0 = RED, 1 = GREEN, 2 = BLUE, 3 = YELLOW

  int ind = get_index("shape", 2, obs_list);
...
USVPose DOCK::forward(const USVPose &goal, float distance){
  USVPose p{0,0,0};
  p.theta = goal.theta;
...
int DOCK::get_index(std::string choice_type, int choice, std::vector<Obstacle> obs_list){
  std::string type_list[] = {"circle", "duck", "plus", "square", "triangle"};
