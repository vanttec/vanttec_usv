...
class XBeeStationNode(XBeeBaseNode):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f"Initialized with topics: {topic_names}")
    ...
    def handle_object_list(self, data: list) -> None:
        ...
        # Process data in groups of 4 (x, y, color, type)
    ...
    def data_received_callback(self, xbee_message):
        ...
        try:
            ...
            self.get_logger().debug(f"Received raw message: {xbee_message.data}")
            
            # Parse message chunk
            msg_id, data = self.parse_message_chunk(xbee_message.data)
            self.get_logger().debug(f"Parsed message ID: {msg_id}, Data: {data}")
            
            # Get topic name safely
            topic_names = list(self.topic_data.keys())
            self.get_logger().debug(f"Available topics: {topic_names}")
            
            if not topic_names:
                raise ValueError("No topics initialized")
            ...
            self.get_logger().debug(f"Selected topic: {topic_name}")
