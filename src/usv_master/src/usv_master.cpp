...
class USVMaster : public rclcpp::Node {
  public:
    USVMaster(): Node("usv_master"), mission(0), mission_status(-1) {
  ...
  private:
    ...
    void xbee_callback(const std_msgs::msg::String & msg)  {
      ...
      if (msg.data[0] == 'm' && mission_status != 1) {
        mission = a[1] - '0';
        RCLCPP_INFO(this->get_logger(), "Mission: '%d'", mission);
      } else {
        RCLCPP_INFO(this->get_logger(), "Invalid command: '%s'", msg.data.c_str());
      }
    ...
    void mission_callback(const std_msgs::msg::Int8 & msg)  {
      ...
      RCLCPP_INFO(this->get_logger(), "Mission status: '%d'", mission_status);
    }
