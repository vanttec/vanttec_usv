...
class AsvOpt:
    def __init__(self):
        ...
        self.T = 50.0  # time horizon [s]
        self.N = 1000  # number of control intervals (K-1 in fatrop notation)
    ...
    def cost(self, uk, xk, k):
        ...
        pos_error = (xk[0] - self.wp_x)**2 + (xk[1] - self.wp_y)**2
        cost_val += self.Q_pos * pos_error

        # Goal heading tracking error (handle angle wrapping)
        heading_error = ca.sin(xk[2] - self.wp_psi)**2 + (1 - ca.cos(xk[2] - self.wp_psi))**2
    ...
    def solve_opt(self, start_state, warm_start_data=None):
        ...
        goal_state = np.array([self.wp_x, self.wp_y, self.wp_psi, 0.0, 0.0])
        # opti, x, u = self.setup_optimization_problem(start_state, goal_state=goal_state, obstacles=obstacles)
        opti, x, u = self.setup_optimization_problem(start_state, goal_state=goal_state)
...
def main():
    ...
    if X_opt is not None:
        ...
        pos_errors = np.sqrt((X_opt[0, :] - wp_x)**2 + (X_opt[1, :] - wp_y)**2)
        heading_errors = np.abs(X_opt[2, :] - wp_psi) * 180/np.pi
