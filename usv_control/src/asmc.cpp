...
class AdaptiveSlidingModeControl{
    public:
        ...
        AdaptiveSlidingModeControl(){
            ...
            right_thruster_pub = n.advertise<Float64>("/usv_control/controller/right_thruster", 1000);
            left_thruster_pub = n.advertise<Float64>("/usv_control/controller/left_thruster", 1000);
            speed_gain_pub = n.advertise<Float64>("/usv_control/asmc/speed_gain", 1000);
            speed_error_pub = n.advertise<Float64>("/usv_control/controller/speed_error", 1000);
            speed_sigma_pub = n.advertise<Float64>("/usv_control/asmc/speed_sigma", 1000);
            heading_sigma_pub = n.advertise<Float64>("/usv_control/asmc/heading_sigma", 1000);
            heading_gain_pub = n.advertise<Float64>("/usv_control/asmc/heading_gain", 1000);
            heading_error_pub = n.advertise<Float64>("/usv_control/controller/heading_error", 1000);
        ...
        void desiredHeadingCallback(const Float64::ConstPtr& _psid){
            psi_d = _psid -> data;
        }
        ...
        void control(){
            if (testing == 1 && arduino == 1){
                ...
                if (abs(psi_d_dif) > 3.141592)
                    psi_d_dif = (psi_d_dif/abs(psi_d_dif))*(abs(psi_d_dif) - 2*3.141592);
    ...
    private:
        ...
        Publisher right_thruster_pub;
        Publisher left_thruster_pub;
        Publisher speed_gain_pub;
        Publisher speed_error_pub;
        Publisher speed_sigma_pub;
        Publisher heading_sigma_pub;
        Publisher heading_gain_pub;
        Publisher heading_error_pub;
        Publisher control_input_pub;

        Subscriber desired_speed_sub;
        Subscriber desired_heading_sub;
        Subscriber ins_pose_sub;
        Subscriber local_vel_sub;
        Subscriber flag_sub;
        Subscriber ardu_sub;
};
