...
def buoy_waypoints(self,buoy_x,buoy_y):
        ...
        w5_x, w5_y = self.gate_to_ned(-5, 0, self.ned_alpha, 0, 0)
...
"""
    def farther(self, dist):
        ...
        @return: --
        '''
        self.target_x, self.target_y = self.gate_to_ned(dist, 0,
                                                        self.ned_alpha,
                                                        self.target_x, 
                                                        self.target_y)
        path_array = Float32MultiArray()
...
def main():
    ...
    while not rospy.is_shutdown() and speedChallenge.activated:
        if speedChallenge.objects_list != last_detection or speedChallenge < 1:
        ...
        if speedChallenge.state == 1:
            ...
            class_list = []
            distance_list = []
            x_list, y_list, class_list, distance_list = challenge_functions.init_lists(speedChallenge)
            ...
            if (len(speedChallenge.objects_list) >= 1):
                ...
                while (not rospy.is_shutdown() and (len(speedChallenge.objects_list)) < 1):
                    if rospy.Time.now().secs - initTime > 1:
                        speedChallenge.farther(speedChallenge, 3)
                        rate.sleep()
                        break
            last_detection = speedChallenge.objects_list
        if speedChallenge.objects_list != last_detection or speedChallenge.state < 4:
            ...
            if speedChallenge.state == 2:
                ...
                if ((len(speedChallenge.objects_list) >= 1) and
                    ...
                    while not rospy.is_shutdown() and (len(speedChallenge.objects_list) < 1 or speedChallenge.objects_list[ind_0]['X'] >= 7):
                        if rospy.Time.now().secs - initTime > 1:
                            speedChallenge.farther(speedChallenge, 4)
                            rate.sleep()
                            break
                last_detection = speedChallenge.objects_list
