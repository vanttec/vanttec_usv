...
def main():
    ...
    while not rospy.is_shutdown() and autoNav.activated:
        ...
        if autoNav.objects_list != last_detection or autoNav.state < 1:
            ...
            # which takes it 5 meters ahead of the gate created by the first pair of buoys
            if autoNav.state == 0:
                autoNav.test.publish(autoNav.state)
        ...
        # State 1: The boat has reached the 5 m mark, and its moving forward until it detects another pair of buoys 0.2 to 5 meters ahead
        if autoNav.state == 1:
            autoNav.test.publish(autoNav.state)
            if len(autoNav.objects_list) >= 2:
                autoNav.state = 2
