...
class NMPC
    ...
    * */
    ...
    usv_control::trajectory trajectory_;
...
public:
    ...
    NMPC(ros::NodeHandle& n)
    {
        ...
        // ROS Publishers 
        desired_traj_pub = n.advertise<usv_control::trajectory>("/mission/trajectory", 1);
        ...
        traj_pub = n.advertise<nav_msgs::Path>("/usv_control/broadcaster/trajectory_plan", 1);

        // ROS Subscribers
        local_vel_sub = n.subscribe("/vectornav/ins_2d/local_vel", 1, &NMPC::velocityCallback, this);
        ins_pos_sub = n.subscribe("/vectornav/ins_2d/NED_pose", 1, &NMPC::positionCallback, this);
		...
		right_thruster_sub = n.subscribe("/usv_control/controller/right_thruster", 1, &NMPC::right_callback, this);
		left_thruster_sub = n.subscribe("/usv_control/controller/left_thruster", 1, &NMPC::left_callback, this);
    ...
    */
    ...
    void waypoint_manager()
    {
        ...
        else
        {
            ...
            trajectory_.trajectory_dot.clear();
            geometry_msgs::Vector3 traj;
            geometry_msgs::Vector3 traj_der;
            traj.x = nedx_callback;
            traj.y = nedy_callback;
            traj.z = 0.0;
            trajectory_.len = 1;
            traj_der.x = 0.0;
            traj_der.y = 0.0;
            traj_der.z = 0.0;
            trajectory_.trajectory.push_back(traj);
            trajectory_.trajectory_dot.push_back(traj_der);
            desired_traj_pub.publish(trajectory_);
        }
    }
    ...
    void control(double _x1, double _y1, double _ak, double _ye)
        {
            ...
            for (int i = 2; i < 202; i++){
                int j = i - 1;
                ocp_nlp_out_get(nlp_config, nlp_dims, nlp_out, j, "x", (void *)acados_out.x1);
                traj_der.x = acados_out.x1[u]*std::cos(acados_out.x1[psi]) - acados_out.x1[v]*std::sin(acados_out.x1[psi]);
                traj_der.y = acados_out.x1[u]*std::sin(acados_out.x1[psi]) + acados_out.x1[v]*std::cos(acados_out.x1[psi]);
                ocp_nlp_out_get(nlp_config, nlp_dims, nlp_out, i, "x", (void *)acados_out.x1);
                traj.x = acados_out.x1[nedx];
                traj.y = acados_out.x1[nedy];
                traj.z = 1.0;
                trajectory_.trajectory.push_back(traj);
                trajectory_.trajectory_dot.push_back(traj_der);
                ...
                pose.pose.position.x = acados_out.x1[nedx];
                pose.pose.position.y = -acados_out.x1[nedy];
